...
export default function NumberCard() {
  ...
  const getPrice =  () => {
    axios.get('https://api.coingecko.com/api/v3/simple/token_price/binance-smart-chain?contract_addresses=0x60e9880daef10e960da3c45b076dbdac4f111edf&vs_currencies=usd&include_24hr_vol=true&include_market_cap=true&include_last_updated_at=true')
        .then(data=> {
          const coin = data?.data['0x60e9880daef10e960da3c45b076dbdac4f111edf'];
          const price = coin?.usd;
          setInfo(info.map( i => {
            if (i.text.includes('Price')) {
              i.number = '$'+price;
            }
            if (i.id == 4) {
              i.number = '$'+coin?.usd_24h_vol?.toFixed(2);
            }
            return i;
          }));
          setTimeout(()=>getPrice(), 5*60*1000)
        })
  }
      ...
      <Container>
        ...
        <Grid sx={styles.grid}>
          {info.map(item => (
            <Container>
              <Box sx={styles.card}>
                <a sx={styles.number}>{item.number}</a>
                <a>{item.img != null ? <img src={item.img} sx={{marginTop:'-10px',marginBottom:'-10px',height: '35px', width: '35px'}}></img> : ''}</a>
                <a sx={{marginBottom:'10px', height: '50px', width: '50px'}}>{item.icon != null ? item.icon : ''}</a>
                <a
                  sx={{
                   // fontWeight: '200px',
                    color: 'text',
